...
\par
We have shown the properties of curves with unit speed. Now given a curve $\al:I\to{\R}^{3}$ with $\norm{\al'}\ne 1$, let $\wb{\al}$ be the arc-length reparametrization of $\al$, so $\norm{\wb{\al}'}=1$. Let $\wb{T}$, $\wb{k}$, $\wb{N}$, $\wb{B}$, and $\wb{\tau}$ be the corresponding functions of $\wb{\al}$. Define the $T$, $\la$, $N$, $B$, and $\tau$ of $\al$ to be those of $\wb{\al}$. We denote the speed of $\al$ is denoted by $v$.
\begin{theorem}[Fernet formulas]
    Let $\al$ be a regular curve on ${\R}^{3}$ with $\ka>0$, then $T'=\ka vN$,  
\end{theorem}
