...
\par
We define the addition, scalar multiplication, dot multiplication, and cross product on vector fields pointwisely. For a vector $Y=\sum{y}_{i}{U}_{i}$ on $\al$, the derivative of $Y$ is defined to be $Y'=\sum(\d{y}_{i}/\d t){U}_{i}$. Let $Y$ and $Z$ be vector fields on a curve $\al$. Fix $t$, rewrite $Y={({y}_{i}(t))}_{\al(t)}$ and $Z={({z}_{i}(t))}_{\al(t)}$. Consider $Y$ as a function ${Y}_{t}:I\to{T}_{\al(t)}({\R}^{3})\iso{\R}^{3}$, then for $a,b\in\R$, $(aY+bZ)'=aY'+bZ'$. We have $(Y\cdot Z)'=(\sum{y}_{i}(t){z}_{i}(t))'=\sum{y}_{i}(t){z}_{i}'(t)+\sum{y}_{i}'(t){z}_{i}(t)=Y\cdot Z'+Y'\cdot Z$ and $(fY)'(t)$
\begin{definition}
