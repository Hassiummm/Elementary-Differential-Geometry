...
\end{proposition}
\begin{proof}
    $(\Ra)$ Let $\be$ be a plane curve, then there exists $p,q\in{\R}^{3}$ such that for all $s\in I$, $(\be(s)-p)\cdot q=0$. Consider $q$ as a constant vector field, then $((\be-p)\cdot q)'=(\be-p)'\cdot q+q'\cdot(\be-p)=\be'\cdot q-p'\cdot q=\be'\cdot q+q'\cdot\be=\be'\cdot q=\be''\cdot q=0$.
\end{proof}
