...
\end{proof}
\begin{proposition}
    Let $F:{\R}^{n}\to{\R}^{m}$ be a mapping. For all $p\in{T}_{p}({\R}^{n})$,  the tangent map ${F}_{*p}:{T}_{p}({\R}^{n})\to{T}_{F(p)}({\R}^{m})$ is a linear map.
\end{proposition}
\begin{proof}
    Rewrite ${F}_{*p}={(v[{f}_{1}],\dots,v[{f}_{m}])}_{F(p)}$, this proposition immediately follows from 
\end{proof}
...
\end{definition}
\par
Recall that ${F}_{*p}$ is a linear map.
