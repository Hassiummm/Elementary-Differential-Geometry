...
\end{proof}
\par
Let $\tau=0$, $\ka>0$, and $\ka'=0$ for some $\al$ with $\norm{\al'}=1$, then $\al$ lies in a circle of radius $1/\ka$ as we proved before. Consider a circle $\al$ in ${\R}^{3}$, it lies in some ${T}_{p}({\R}^{2})$ by definition, so $\al$ is a plane curve and $\tau=0$. Take the constant $B(0)$ as $q$ in the plane, then 


Define $R:I\to{\R}^{3}$ by $R(s)=\al(s)-c$, where $c$ is the center. We have $(R\cdot R)'=2R'\cdot R=2T\cdot R=0$.
\begin{definition}
    Let $\al:I\to{\R}^{3}$ be a cylindrical helix with the corresponding unit vector $u$. We say $\al$ is a \hdef{circular helix} if its projection onto any plane orthogonal to the axis direction $u$ is a circle.
\end{definition}
\par
