...
\begin{definition}
    Let $f$ be a differetiable real-valued function on ${\R}^{3}$ and let ${v}_{p}$ be a tangent vector on ${\R}^{3}$. The \hdef{directional derivative} of $f$ with respect to ${v}_{p}$, denoted ${v}_{p}[f]$, is defined to be $\frac{\mathrm{d}}{\d t}f(p+tv)$ at $t=0$.
\end{definition}
\begin{remark}
    We will not write the restriction every time for convenience.
\end{remark}
\begin{proposition}
    Let ${v}_{p}={({v}_{1},{v}_{2},{v}_{3})}_{p}$ be a tangent vector, then ${v}_{p}[f]=\sum{v}_{i}\frac{\pa f}{\pa{x}_{i}}(p)$.
\end{proposition}
\begin{proof}
    Let $p=({p}_{1},{p}_{2},{p}_{3})$. Then ${v}_{p}[f]=\frac{\d}{\d t}f(p+tv){\vert}_{t=0}=\sum\frac{\pa f}{\pa z}(p)\cdot\frac{\d}{\d t}({p}_{i}+t{v}_{i})=\sum\frac{\pa f}{\pa{x}_{i}}(p){v}_{i}$.
\end{proof}
\begin{example}
    Consider $f={x}^{2}yz$ with $p=(1,1,0)$ and $v=(1,0,-3)$. By the definition, $p+tv=(1+t,1,-3t)$, so ${v}_{p}[f]=\frac{\d}{\d t}(-3{t}^{3}-6{t}^{2}-3t)=-3$. By the previous proposition, since $\frac{\pa f}{\pa x}=2xyz$, $\frac{\pa f}{\pa y}={x}^{2}z$, and $\frac{\pa f}{\pa z}={x}^{2}y$, we have $\frac{\pa f}{\pa x}(p)=\frac{\pa f}{\pa y}(p)=0$ and $\frac{\pa f}{\pa z}(p)=1$, so ${v}_{p}[f]=-3$.
\end{example}
\begin{proposition}
...
\end{proposition}
\begin{proof}
    (\rom1) We have $(a{v}_{p}+b{w}_{p})[f]=\sum(a{v}_{i}+b{w}_{i})\frac{\pa f}{\pa{x}_{i}}(p)=\sum a{v}_{i}\frac{\pa f}{\pa{x}_{i}}+\sum b{w}_{i}\frac{\pa f}{\pa{x}_{i}}(p)=a{v}_{p}[f]+b{w}_{p}[f]$. (\rom2) We have ${v}_{p}[af+bg]=\sum{v}_{i}\frac{\pa(af+bg)}{\pa{x}_{i}}(p)=\sum{v}_{i}\frac{\pa(af)}{\pa{x}_{i}}(p)+\sum{v}_{i}\frac{\pa(bg)}{\pa{x}_{i}}(p)=a{v}_{p}[f]+b{v}_{p}[g]$. (\rom3) We have ${v}_{p}[fg]=\sum{v}_{i}\frac{\pa(fg)}{\pa{x}_{i}}(p)=\sum{v}_{i}\frac{\pa f}{\pa{x}_{i}}(p)g(p)+f(p)\sum{v}_{i}\frac{\pa g}{\pa{x}_{i}}(p)={v}_{p}[f]g(p)+f(p){v}_{p}[g]$.
\end{proof}
\par
...
\end{definition}
\begin{example}
    A \hdef{straight line} is a curve $\al:\R\to{\R}^{3}$ defined by $\al(t)=p+tq$, where $\al(0)=p$ and $q\ne 0$.
\end{example}
\begin{example}
    A \hdef{helix} is a curve $\al:\R\to{\R}^{3}$ defined by $\al(t)=(a\cos t,a\sin t,bt)$ for all $a>0$ and $b\ne 0$.
\end{example}


\begin{definition}
    Let $\al:I\to{\R}^{3}$ be a curve with $\al=({\al}_{1},{\al}_{2},{\al}_{3})$. For all $t\in I$, the \hdef{velocity vector} of $\al$ at $t$ is the tangent vector $\al'(t)={(\frac{\d{\al}_{1}}{\d t}(t),\frac{\d{\al}_{2}}{\d t}(t),\frac{\d{\al}_{3}}{\d t}(t))}_{\al(t)}$ at the point $\al(t)\in{\R}^{3}$.
\end{definition}
