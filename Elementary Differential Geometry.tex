...
\end{remark}
\begin{definition}
    Let $\be:I\to{\R}^{3}$ be a curve. Then we call $N=T'/\ka$ the \hdef{principal normal vector field} of $\be$. The vector field $\be=T\times N$ on $\be$ is called the \hdef{binormal vector field} of $\be$.
\end{definition}
\begin{proposition}
...
\end{proof}
\begin{definition}
    Let $\be$ be a curve in ${\R}^{3}$ with $\ka>0$ and $\norm{\be'}=1$. Then $(T,N,B)$ is called the \hdef{Fernet frame field} of $\be$.
\end{definition}
\par
...
\end{example}
\begin{definition}
    Let $p,q\in{\R}^{3}$ with $q\ne 0$. The \hdef{plane} through $p$ orthogonal to $q$ is the set $\{r\in{\R}^{3}\mid(r-p)\cdot q\}=0$. A curve $\be:I\to{\R}^{3}$ is said to be a \hdef{plain curve} if $\ran(\be)\sub P$, where $P$ is a plane in ${\R}^{3}$.
\end{definition}
\begin{proposition}
    Let $\be:I\to{\R}^{3}$ be a curve with $\norm{\be'}=1$ and $\ka>0$. Then $\be$ is a plane curve if and only if $\tau=0$. 
\end{proposition}
\begin{proof}
    $(\Ra)$ Let $\be$ be a plane curve, then there exists $p,q\in{\R}^{3}$ such that for all $s\in I$, $(\be(s)-p)\cdot q=0$. Consider $q$ as a constant vector field, then $((\be-p)\cdot q)'=(\be-p)'\cdot q+q'\cdot(\be-p)=\be'\cdot q-p'\cdot q=\be'\cdot q+q'\cdot\be=\be'\cdot q=\be''\cdot q=0$. Since 
\end{proof}
