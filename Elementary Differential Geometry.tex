...
\end{proof}
\par
Let a regular curve $\al$ on ${\R}^{3}$ with $\norm{\al'}=1$ be a circle. Let $C$ be the center and let $R:I\to{\R}^{3}$ be the function defined by $R(s)=\al(s)-C$.


Let $\tau=0$, $\ka>0$, and $\ka'=0$ for some $\al$ with $\norm{\al'}=1$. By the Fernet formulas, $B'=0$ and $N'=-\ka T$. Define $C(s)=\al(s)+N(s)/\ka$, then $C'(s)=T(s)+N'(s)/\ka=T(s)-\ka T(s)/\ka=0$, so $C$ is constant. Let $C$ be the center, then for all $s$, $\norm{\al(s)-C}=1/\ka$.
\begin{definition}
    Let $\al:I\to{\R}^{3}$ be a cylindrical helix. We say $\al$ is a \hdef{circular helix} if its projection onto any plane orthogonal to the axis direction $u$ is a circle.
\end{definition}
\par
